...
# Replace 'YOUR_YOUTUBE_URL' with the actual YouTube streaming URL
youtube_url = "https://www.youtube.com/live/ZxL5Hm3mIBk?feature=shared"
...
def detect_objects(frame):
    ...
    detections = np.concatenate((boxes, confidences[:, np.newaxis], class_ids[:, np.newaxis]), axis=1)  # Concatenate detection information
    return detections
...
# Use FFmpeg to open the YouTube live stream
ffmpeg_cmd = [
    'ffmpeg',
    '-i', stream_url,
    '-f', 'image2pipe',
...
while True:
    ...
    frame_size = 1920 * 1080 * 3  # Adjusted for 1080p
    ...
    for track in tracker.tracks:
        ...
        # Bounding box coordinate and color setting
        ...
        # Speed and direction calculation (Center point coordinate)
        ...
        if track_id in previous_positions:
            prev_bottom_pos, prev_time = previous_positions[track_id][-1]  # Get previous positions
        ...
        previous_positions[track_id].append((current_bottom_pos, start.timestamp()))  # Convert the current time to a timestamp and save it
