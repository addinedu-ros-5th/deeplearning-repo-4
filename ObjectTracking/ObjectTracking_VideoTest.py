...
video_file_path = "/Users/yohan/Desktop/dev_ws/DeepLearning_Project/Object-Detection-and-Tracking-with-YOLOv8-and-DeepSORT/data/record5.mov"
video_cap = cv2.VideoCapture(video_file_path)

# Automatically sets the frame size and FPS of the video
...
# Initialize the YOLOv8 model using the default weights and move it to the device
...
def detect_objects(frame):
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # OpenCV uses BGR format by default, convert it to RGB
    results = model(frame)  # YOLOv8 model inference
    results = results[0]  # Get the first batch of results
